...
log = logging.getLogger("acss_udp_client")


class ACUDPListener(object):
    def __init__(self):
        pass


class ACUDPClient4(object):
    @classmethod
    ...
    def read_event(self):
        event = ACUDPClient4.consume_event(self.file)
        if event:
            for subs in self._subscribers.itervalues():
                type_ = ACUDPProtoTypes4.id_to_name(event['type'])
    ...
    def broadcast_message(self, message):
        ...
        data = struct.pack("BB%ds" % (size*4,),
            ACUDPProtoTypes4.ACSP_BROADCAST_CHAT,
            size,
    ...
    def send_message(self, car_id, message):
        ...
        data = struct.pack("BBB%ds" % (size*4,),
            ACUDPProtoTypes4.ACSP_SEND_CHAT,
            car_id,
            size,
            message.encode('utf32')
        )
